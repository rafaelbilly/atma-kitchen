...
import { getOnProcessTransactionToday } from "../../../lib/repository/TransactionRepository";
import { MOTaskTable } from "../../../components/Table/Table";
import { ConfirmProcessMOModal } from "../../../components/Modal";
...
const ConfirmedOrder: React.FC = () => {
  ...
  const renderModal = (data: any[], status: string) => {
    if (data.length > 0) {
      ...
      switch (status) {
        case "Confirmed":
          return <ConfirmProcessMOModal data={selectedData} />;
        default:
          return null;
      }
    }
